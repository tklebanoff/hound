...
fn read_append<W: io::Read + io::Seek>(mut reader: &mut W) -> Result<(WavSpecSurge, u32, u32)> {
    ...
    let spec = surge_spec.specx.unwrap().spec;
    ...
    let supported = match (surge_spec.specx.unwrap().bytes_per_sample, spec.bits_per_sample) {
        (1, 8) => true,
        (2, 16) => true,
        (3, 24) => true,
        (4, 32) => true,
...
impl WavWriter<io::BufWriter<fs::File>> {
    ...
    pub fn append<P: AsRef<path::Path>>(filename: P) -> Result<WavWriter<io::BufWriter<fs::File>>> {
        ...
        let writer = WavWriter {
            spec: surge_spec.specx.unwrap().spec,
